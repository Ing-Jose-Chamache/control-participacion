...
# Inicialización del ID de sesión (CORREGIDO)
if 'session_id' not in st.session_state:
    # Usar un valor basado en el usuario actual o generar uno nuevo
    st.session_state.session_id = str(uuid.uuid4())
    
    # Intentar guardar el ID en un archivo persistente
    try:
        os.makedirs('.streamlit', exist_ok=True)
        with open('.streamlit/cookie_store.json', 'w') as f:
            json.dump({'session_id': st.session_state.session_id}, f)
    except Exception as e:
        # Si falla, seguimos usando el ID generado
        pass
...
def load_state():
    try:
        ...
        st.warning(f"Error al cargar el estado: {e}")
        return False
...
if 'state_loaded' not in st.session_state:
    # Marcar que ya intentamos cargar el estado
