...
def get_persistent_session_id():
    # Si ya hay un ID en session_state, lo usamos
    if 'session_id' in st.session_state:
        return st.session_state.session_id
    
    # Intentamos obtener ID de cookies o parámetros URL (implementación simplificada)
    # En una implementación real, esto se haría con cookies HTTP o tokens
    # Para Streamlit Cloud, usamos un UUID aleatorio que será constante durante la sesión
    session_id = str(uuid.uuid4())
    
    # Guardamos el ID para futuros accesos
    st.session_state.session_id = session_id
    return session_id

# Inicializar ID de sesión
...
st.sidebar.markdown(f"""
    ...
    '>
        <span style='color: #555;'>ID:</span> <span style='color: #0066cc; font-weight: bold;'>{session_id[:8]}...</span>
    </div>
...
def save_state():
    ...
    try:
        ...
        with open(f'sesiones/app_state_{session_id}.json', 'w') as f:
...
def load_state():
    try:
        ...
        session_file = f'sesiones/app_state_{session_id}.json'
...
def reset_state():
    ...
    if os.path.exists(session_file):
        try:
            ...
            st.error(f"Error al eliminar archivo de sesión: {e}")
            
    # Reiniciar variables en session_state
...
# Inicialización del estado SOLO SI no existe ya
...
# Cargar estado al inicio - CORREGIDO
if 'state_loaded' not in st.session_state:
    # Marcar que ya intentamos cargar el estado (una sola vez por sesión)
...
if st.button("🗑️ Reiniciar Todo"):
    reset_state()
    st.experimental_rerun()  # Usar experimental_rerun en lugar de rerun para forzar recarga
...
if st.button("AGREGAR ESTUDIANTE") and nuevo_estudiante:
    if nuevo_estudiante not in st.session_state.estudiantes['Nombre'].values:
        ...
        save_state()
        st.experimental_rerun()  # Forzar recarga para ver el cambio
...
for idx, estudiante in st.session_state.estudiantes.iterrows():
    ...
    with col4:
        # Botón para eliminar estudiante - CORREGIDO
        if st.button("🗑️", key=f"delete_{estudiante['Nombre']}"):
            # Crear una copia del DataFrame excluyendo el estudiante a eliminar
            st.session_state.estudiantes = st.session_state.estudiantes[st.session_state.estudiantes['Nombre'] != estudiante['Nombre']].reset_index(drop=True)
            save_state()
            st.experimental_rerun()
    st.markdown("</div>", unsafe_allow_html=True)
