...
def export_excel():
    ...
    # Crear un DataFrame con las estadísticas detalladas
    ...
    # Añadir estado (APROBADO, REGULAR, DESAPROBADO)
    df_stats['Estado'] = df_stats['Nota_Vigesimal'].apply(
        lambda nota: "APROBADO" if nota >= 14 else ("REGULAR" if nota >= 11 else "DESAPROBADO")
    )
    
    # Añadir detalle de respuestas por pregunta (columnas adicionales)
    for i in range(st.session_state.num_preguntas):
        df_stats[f'Pregunta_{i+1}'] = st.session_state.estudiantes['Respuestas'].apply(
...
st.markdown("""
    ...
    /* Nuevo estilo para el botón de exportar Excel */
    .export-excel-button {
        display: inline-block;
        margin-left: 15px;
