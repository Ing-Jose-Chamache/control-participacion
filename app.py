...
if 'num_preguntas' not in st.session_state:
    st.session_state.num_preguntas = 5
# Sección de header con logo y ruleta
st.markdown('<div class="header-section">', unsafe_allow_html=True)

# Lado izquierdo - Logo
st.markdown('<div style="width:373px;">', unsafe_allow_html=True)
logo_file = st.file_uploader("", type=['png', 'jpg', 'jpeg'], key="logo")
if logo_file:
    st.image(logo_file, width=373)
st.markdown('</div>', unsafe_allow_html=True)

# Lado derecho - Ruleta y su botón de carga
st.markdown('<div class="ruleta-container">', unsafe_allow_html=True)
ruleta_file = st.file_uploader("", type=['txt'], key="ruleta")
components.html("""
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        .ruleta-wrapper {
            width: 447px;  /* 20% más grande que el logo (373 * 1.2) */
            height: 447px;
            position: relative;
        }
    </style>
...
<body>
    ...
    <script type="text/babel">
        const RuletaPreviewFinal = () => {
          ...
          const anglePerSection = nombres.length > 0 ? 360 / nombres.length : 360;
          ...
          React.useEffect(() => {
            // Esta función se llamará cuando el archivo se cargue
            window.actualizarNombresRuleta = (nuevosNombres) => {
            ...
            <div className="ruleta-wrapper">
              ...
              >
                {nombres.length > 0 ? (
                  nombres.map((nombre, index) => {
                    ...
                    const pathData = `M 50 50 L ${startX} ${startY} A 45 45 0 ${
                      anglePerSection <= 180 ? "0" : "1"
                    } 1 ${endX} ${endY} Z`;
                  ...
                  <circle cx="50" cy="50" r="45" fill="#f0f0f0" stroke="#cccccc" />
                )}
              ...
              <button 
                ...
                style={{
                  ...
                  padding: '10px 20px',
                  backgroundColor: isSpinning || nombres.length === 0 ? '#ccc' : '#0066cc',
                  color: 'white',
                  border: 'none',
                  borderRadius: '5px',
                  cursor: nombres.length === 0 ? 'not-allowed' : 'pointer'
                }}
              >
...
# Script para cargar nombres desde el archivo
if ruleta_file:
    contenido = StringIO(ruleta_file.getvalue().decode("utf-8")).read().splitlines()
    nombres_ruleta = [nombre.strip() for nombre in contenido if nombre.strip()]
    st.markdown(
        f"""
        <script>
            if (window.actualizarNombresRuleta) {{
                window.actualizarNombresRuleta({nombres_ruleta});
            }}
        </script>
        """,
        unsafe_allow_html=True
...
# Agregar un divisor después del header
