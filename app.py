...
st.markdown("""
    ...
    /* Estilos para el reloj de Perú */
    .peru-clock {
        position: absolute;
        right: 50px;
        top: 70px; /* Posición ajustada más arriba */
        transform: none; /* Eliminamos la transformación vertical */
        background-color: #ffffff;
        border-radius: 10px;
        padding: 10px 15px;
...
# El botón de AnyDesk se moverá a la parte 3 debajo de "Respuestas Correctas de Estudiantes"
...
# Reloj de Perú (en la parte derecha)
st.markdown(
    """
    <div class="peru-clock">
        <div class="clock-title">Hora - Perú</div>
        <div class="clock-time" id="peru-time">12:00:00</div>
        <div class="clock-date" id="peru-date">01/01/2025</div>
        <script>
            function updateClock() {
                const now = new Date();
                // Ajustar a hora de Perú (UTC-5)
                const peruOptions = { timeZone: 'America/Lima', hour12: false };
                const peruTimeString = peruTime.toLocaleTimeString('es-PE', peruOptions);
                const peruDateString = peruTime.toLocaleDateString('es-PE', { timeZone: 'America/Lima' });
                ...
                document.getElementById('peru-time').textContent = `${hours}:${minutes}:${seconds}`;
                document.getElementById('peru-date').textContent = `${day}/${month}/${year}`;
...
st.markdown(
    f'<div class="ruleta-link" style="right: 220px; top: 70px; transform: none;">'
