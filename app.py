...
# Nueva función: Exportar estadísticas en CSV
def export_stats_csv():
    if not st.session_state.estudiantes.empty:
        ...
        df_stats = pd.DataFrame({
            ...
            'Nota_Vigesimal': st.session_state.estudiantes['Respuestas'].apply(lambda x: round((sum(1 for r in x if r == '1') / st.session_state.num_preguntas) * 20, 1)),
            'Estado': st.session_state.estudiantes['Respuestas'].apply(lambda x: 
                "APROBADO" if ((sum(1 for r in x if r == '1') / st.session_state.num_preguntas) * 20) >= 14 
                else ("REGULAR" if ((sum(1 for r in x if r == '1') / st.session_state.num_preguntas) * 20) >= 11 
                     else "DESAPROBADO"))
        })
...
st.markdown("""
    ...
    /* Estilos para botones de IA y AnyDesk */
    .ai-buttons-container {
        position: fixed;
        top: 20px;
        right: 140px; /* Ajustado para no solapar con el botón de reinicio */
        z-index: 1000;
        display: flex;
        gap: 10px;
    }
    ...
    /* Estilos para el botón de exportar CSV */
    .download-btn {
        display: inline-block;
        padding: 10px 20px;
...
st.markdown("""
    <div class="ai-buttons-container">
        <a href="https://chatgpt.com/" target="_blank" class="ai-button chatgpt-button" title="Abrir ChatGPT">
            <img src="https://github.com/Ing-Jose-Chamache/imagenesjch/blob/main/chatgpt-icon.png?raw=true" alt="ChatGPT">
        </a>
        <a href="https://claude.ai/" target="_blank" class="ai-button claude-button" title="Abrir Claude II">
            <img src="https://github.com/Ing-Jose-Chamache/imagenesjch/blob/main/claude-icon.png?raw=true" alt="Claude II">
        </a>
        <a href="anydesk:" class="ai-button anydesk-button" title="Abrir AnyDesk">
            <img src="https://github.com/Ing-Jose-Chamache/imagenesjch/blob/main/anydesk-icon.png?raw=true" alt="AnyDesk">
        </a>
    </div>
""", unsafe_allow_html=True)
